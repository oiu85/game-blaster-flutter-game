...
## Project Structure
* **Standard Structure:** our projectes are follow we follow the Design Principles of Flutter in our case is DDD
...
* **State Management:** Separate ephemeral state and app state. Use a state
  management solution for app state to handle the separation of concerns the  best thing is to use BLOC.
...
* **Exception Handling:** Use `Either Package` blocks for handling exceptions, and
...
* **Separation of Concerns:** Aim for separation of concerns similar to DDD Domain Driven Design,
  Presentation, Domain for abstracting the repository, and Data roles for the model and the repository implementation.
* **Logical Layers:** Organize the project into logical layers:
    * Presentation (widgets, screens)
    * Domain (abstract business logic classes)
    * Data (model classes, and repository implementations)
...
* **DDD:** When a more robust solution is needed, structure the app using the
  Domain-Driven Design (DDD) pattern.
* **Dependency Injection:** Use the package GetIt for simple manual constructor dependency injection
...
* **Responsiveness:** Ensure the app is mobile responsive and adapts to
  different screen sizes, working perfectly on mobile and web For that use The package `flutter_screenutil`.
...
* **Custom Fonts:** For custom fonts, use the `google_fonts` package. Define a
  ...
  // 2. Define a TextTheme with a custom font with screen util
  final TextTheme appTextTheme = TextTheme(
    displayLarge: GoogleFonts.oswald(fontSize: 57.sp, fontWeight: FontWeight.bold),
    titleLarge: GoogleFonts.roboto(fontSize: 22.sp, fontWeight: FontWeight.w500),
...
* **Generated Code:** Use `flutter pub run build_runner build` to generate code
  for assets.
...
* **Use `///. //? , //! , //* , //TODO: ` for doc comments:** This allows documentation generation tools to
  pick them up.
